...
class ExposableTypeMap {
	 ...
	 * @return string
	 * @throws \TYPO3\Flow\Property\Exception\FormatNotSupportedException
	 */
	public function getType($classIdentifier) {
		if (array_key_exists($classIdentifier, $this->classIdentifierToTypeNameMap)) {
			...
			throw new \TYPO3\Flow\Property\Exception\FormatNotSupportedException('There is no target type for class name "' . $classIdentifier . '"', 1451995790);
		}
	}
	...
	public function getClassName($typeName) {
		if (array_key_exists($typeName, $this->typeNameToClassIdentifierMap)) {
			...
			throw new \TYPO3\Flow\Property\Exception\FormatNotSupportedException('There is no target class name for type "' . $typeName . '"', 1451995976);
		}
	}
